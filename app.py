...
def get_posts(author):
    ...
    return (
        jsonify({"posts": list_of_posts}, 200)
        if list_of_posts
        else jsonify({"message": "No posts found"}, 404)
    )
...
def create_post(author):
    try:
        ...
        return jsonify(
            {"message": "Something went wrong while creating the post."}, 404
        )
...
def create_author(author):
    try:
        ...
        return jsonify(
            {"message": f"Author [{response.name}] created successfully"}, 200
        )
    except:
        return jsonify(
            {"message": "Something went wrong while creating the author."}, 400
        )
...
def update_author(author, id_author):
    ...
    return jsonify({"message": f"Author [{author.name}] updated successfully"}, 200)
...
def delete_author(author, id_author):
    ...
    return jsonify(
        {"message": f"Author [{author_to_delete.name}] deleted successfully"}, 200
    )
